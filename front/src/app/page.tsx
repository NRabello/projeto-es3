...
export default function Home() {
  ...
  useEffect(() => {
    setBookList(books.filter((book) => 
      ...
      (book.active ? "true" : "false") === search.toLowerCase()
    ));
  }, [search]);
  ...
  return (
    <div className="bg-gradient-to-r from-pink-600 to-cyan-700 min-h-screen">
      <div className="flex flex-col items-center">
        <h1 className="mx-auto w-fit h-8 mt-10 bg-clip-text text-transparent bg-gradient-to-r from-purple-900 to-red-700 text-3xl font-semibold leading-7 ">All Books</h1>
        <div className="shadow-md sm:rounded-lg mt-4 w-8/12">
          <div className="flex justify-between">
            <input placeholder="Pesquise por Titulo, Autor, Categoria, Editora ou Status" className="bg-gray-700 text-gray-400 px-2 py-1 w-10/12 mt-2 mb-2 mr-1 rounded-md" onChange={(e) => setSearch(e.target.value)} />
            <button onClick={() => router.push("/create")} className="w-48 bg-gray-700 mt-2 mb-2 text-gray-400 px-2 py-1 rounded-md hover:bg-green-800">Adicionar Livro</button>
          </div>
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {bookList.map((book, index) => (
                <tr onClick={() => router.push("/update")} key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 cursor cursor-pointer">
                  <td className="px-6 py-4">{book.title}</td>
                  <td className="px-6 py-4">{book.author}</td>
                  <td className="px-6 py-4">{book.category}</td>
                  <td className="px-6 py-4">{book.publisher}</td>
                  <td className="px-6 py-4">{book.active ? "Active" : "Inactive"}</td>
